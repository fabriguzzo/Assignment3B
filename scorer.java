...
class Scorer {
    ...
    public List<Result> scoreAndRank(Preferences p) {
        ...
        for (Car c : cars) {
            ...
            else b.sportyN = (p.getSportyLook() == c.isSportyLook()) ? 1.0 : 0.0;   // if it is sporty and user wants sporty, give 1.0
            ...
            if (p.hasFuelFilter()) {
                boolean fuelMatch = p.getFuelTypes().stream() // retrieve the fuel types from preferences
                        .anyMatch(f -> f.equalsIgnoreCase(c.getFuelType())); // should be equalsIgnoreCase
